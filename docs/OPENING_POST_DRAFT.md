...
Garmon — это ранний исследовательский прототип о телесном смысле события перед речью LLM.

Главная идея простая: ответ не должен начинаться только с генерации текста. Перед финальными словами может быть более ранний слой, где событие получает смысл.
